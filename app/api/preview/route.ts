...
export async function POST(req: NextRequest) {
  ...
  const tmpFile = join(tmpdir(), `shiro-preview-${Date.now()}.json`)
  ...
  try {
    const { stdout, stderr } = await execAsync(
      `shiro run -dry-run -workflow "${tmpFile}"`,
      {
        timeout: 10000,
      }
    )
    ...
    // shiro run -dry-run exits non-zero on validation failure
    ...
    if (
      ...
      return NextResponse.json(
        {
          error:
            "shiro binary not found in PATH. Install Shiro to use dry-run preview.",
          fallback: true,
        },
        { status: 503 }
      )
